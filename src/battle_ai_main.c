...
// the following checks apply to any target (including user)
    ...
    case EFFECT_GEAR_UP:
        ...
        {
            ADJUST_SCORE(-10);    // no partner and our stats won't rise, so don't use
        }
        break;
    ...
    case EFFECT_MAGNETIC_FLUX:
        ...
        {
            ADJUST_SCORE(-10);    // our stats won't rise from this move
        }
        break;
...
static s32 AI_FirstBattle(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    if (!isFrlg && gAiLogicData->hpPercents[battlerDef] <= 20)
        AI_Flee();
